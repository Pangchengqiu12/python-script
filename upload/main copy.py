...
def main():
    ...
    for idx in range(start_row, len(df)):
        ...
        try:
            ...
            sncode = str(row[col_letter_to_index(COLUMN_MAPPING["sncode"])]).strip()
            equipmentName = str(
                row[col_letter_to_index(COLUMN_MAPPING["equipmentName"])]
            ).strip()
            location = str(row[col_letter_to_index(COLUMN_MAPPING["location"])]).strip()
            longitude = float(row[col_letter_to_index(COLUMN_MAPPING["longitude"])])
            latitude = float(row[col_letter_to_index(COLUMN_MAPPING["latitude"])])
